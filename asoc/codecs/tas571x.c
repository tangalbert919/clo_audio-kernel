			     ...
			     struct snd_soc_dai *dai)
{
	...
	u32 val;
	int ret;
	ret = regmap_read(priv->regmap,TAS571X_SDI_REG, &val);
	if(!ret) { 
		pr_err("I2C read succss ret: %d   Val: %d\n", ret, val);
		return ret;
	}
	else
				  ...
				  enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_STANDBY:
		if (snd_soc_component_get_bias_level(component) == SND_SOC_BIAS_OFF) {
			ret = msm_cdc_pinctrl_select_active_state(priv->wsa_clk_gpio_p);
			if (ret) {
	...
	case SND_SOC_BIAS_OFF:
		...
		ret = msm_cdc_pinctrl_select_sleep_state(priv->wsa_clk_gpio_p);
		if (ret) {
			     ...
			     const struct i2c_device_id *id)
{
	...
	priv->mclk = devm_clk_get(dev, "wsa_mclk");
